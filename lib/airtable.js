...
// Helper to parse Add-ons JSON from Airtable Long Text field
...
// Helper to parse JSON array from Airtable Long Text field
...
export async function getAllTechnicians() {
  try {
    ...
    return records.map(record => ({
      ...
      'Plumbing Rating': record.fields['Plumbing Rating'] || 0,
      'Electrical Rating': record.fields['Electrical Rating'] || 0
    }))
  } catch (error) {
  ...
  try {
    const records = await base(process.env.AIRTABLE_JOBS_TABLE)
      .select({
        ...
        // Show all past jobs + jobs up to 45 days ahead, plus unscheduled jobs (no date)
        filterByFormula: 'OR(IS_BEFORE({Target Date}, DATEADD(TODAY(), 46, "days")), {Target Date} = BLANK())'
      })
      .all()
    ...
    return records.map(record => ({
      ...
      serviceDetail: record.fields['Service Detail'] || null,  // For plumbing/electrical sub-services
      complexity: record.fields['Complexity'] || null,         // Quoted estimate
      confirmedComplexity: record.fields['Confirmed Complexity'] || null,  // Tech confirms on-site
      date: record.fields['Target Date'],
      status: record.fields['Status'],
      customerName: record.fields['Customer'],
      address: record.fields['Street Address (from Quote ID) (from Quote Line Item)'] || record.fields['Property Address'],
      ...
      estimatedTime: record.fields['Expected Time to Complete'] || null,
      endTime: record.fields['Scheduled End'] || null,
      ...
      // Add-ons/Change Orders (stored as JSON string in Long Text field)
      addOns: parseAddOns(record.fields['Add-ons']),
      // Additional Services (multi-select field for adding services to existing jobs)
      additionalServices: record.fields['Additional Services'] || [],
      // Job Materials (separate fields for clean data)
      materials: parseMaterials(
        record.fields['Materials Vendors'],
        record.fields['Materials Amounts'],
        record.fields['Materials Descriptions']
      ),
      materialReceipts: record.fields['Material Receipts'] || [],
      allowTechTexting: record.fields['Allow Tech Texting'] || false
    }))
  } catch (error) {
...
export async function updateJob(jobId, updates) {
  try {
    ...
    if (updates.endTime !== undefined) {
      fields['Scheduled End'] = updates.endTime
    }
    ...
    // Confirmed complexity (tech confirms on-site for plumbing/electrical)
    if (updates.confirmedComplexity !== undefined) {
      fields['Confirmed Complexity'] = updates.confirmedComplexity
    }
    ...
    // Add-ons/Change Orders (stored as JSON string)
    if (updates.addOns !== undefined) {
      fields['Add-ons'] = JSON.stringify(updates.addOns)
    }
    // Additional Services (multi-select)
    if (updates.additionalServices !== undefined) {
      fields['Additional Services'] = updates.additionalServices
    }
    if (updates.allowTechTexting !== undefined) {
      fields['Allow Tech Texting'] = updates.allowTechTexting
    }
    // Job Materials (separate fields)
    if (updates.materials !== undefined) {
      const materials = updates.materials
      fields['Materials Vendors'] = JSON.stringify(materials.map(m => m.vendor))
      fields['Materials Amounts'] = JSON.stringify(materials.map(m => m.amount))
      fields['Materials Descriptions'] = JSON.stringify(materials.map(m => m.description))
    }
...
export async function getPricingRules() {
  try {
    ...
    return records.map(record => ({
      id: record.id,
      serviceName: record.fields['Service'],
      serviceDetail: record.fields['Service Detail'] || null,
      complexity: record.fields['Complexity'] || null,
...
// Helper to format phone number to +1XXXXXXXXXX format
function formatPhoneForStorage(phone) {
  ...
  // Return cleaned if already has country code
...
export async function createTechnician(data) {
  try {
    const fields = {
      'Name': `${data.firstName} ${data.lastName}`, // Combined name (primary field)
      'First Name': data.firstName,
      'Last Name': data.lastName,
    ...
    if (data.emergencyPhone) fields['Emergency Phone'] = formatPhoneForStorage(data.emergencyPhone)
