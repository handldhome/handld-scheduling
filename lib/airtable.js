...
export async function getEquipmentOptions() {
  try {
    ...
    // Find the Jobs table
    const jobsTable = data.tables.find(t => t.name === process.env.AIRTABLE_JOBS_TABLE)
    if (!jobsTable) {
      throw new Error('Jobs table not found')
    }

    // Find the Requires Equipment field
    const equipmentField = jobsTable.fields.find(f => f.name === 'Requires Equipment')
...
// Get list of services from Jobs table Service field
export async function getServiceOptions() {
  try {
    ...
    // Find the Service field (should be single select or multi-select)
    const serviceField = jobsTable.fields.find(f => f.name === 'Service')
    if (!serviceField || !serviceField.options?.choices) {
      return []
    }

    return serviceField.options.choices.map(choice => ({
      id: choice.id,
      name: choice.name,
      color: choice.color
    }))
...
export async function createTechnician(data) {
  try {
    ...
    if (data.services && data.services.length > 0) fields['Services'] = data.services
    if (data.equipment && data.equipment.length > 0) fields['Equipment Owned'] = data.equipment
