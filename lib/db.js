...
export async function getAllJobs() {
  ...
  const { data, error } = await db
    .from('jobs')
    .select('*, technician:technicians(id, airtable_id, first_name, last_name), quote_request:quote_requests(address, city, square_footage, lot_size, stories)')
  ...
  return data.map(j => ({
    ...
    phone: '', // populated from customer if needed
    assignedTech: j.technician ? `${j.technician.first_name || ''} ${j.technician.last_name || ''}`.trim() : '',
    assignedTechId: j.technician_id,
    notes: j.other_notes || '',
...
export async function updateJob(jobId, updates) {
  ...
  if (updates.assignedTech !== undefined) {
    // assignedTech could be a name or ID — resolve to UUID
    if (updates.assignedTech) {
      const techUuid = await resolveTechName(updates.assignedTech)
  ...
  if (updates.date !== undefined) fields.target_date = updates.date
  if (updates.notes !== undefined) fields.other_notes = updates.notes
  if (updates.time !== undefined) fields.scheduled_time = updates.time
  if (updates.endTime !== undefined) fields.scheduled_end = updates.endTime
