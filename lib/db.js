...
export async function getAllJobs() {
  ...
  return data.map(j => ({
    ...
    techConfirmationDeadline: j.tech_confirmation_deadline || null,
    techConfirmedAt: j.tech_confirmed_at || null,
    techDeclinedAt: j.tech_declined_at || null,
    techDeclineReason: j.tech_decline_reason || ''
  }))
}
...
export async function saveAvailability(techId, availabilityData, dayNotes = {}) {
  ...
  // Delete existing availability for the exact dates being submitted
  // (avoids timezone mismatch between client local time and server UTC)
...
export async function getEquipmentOptions() {
  // Return hardcoded equipment options
  return [
 ...
 * @param {{ jobId?: string, recipientPhone: string, recipientType: 'customer'|'technician', messageType: string, messageBody: string, twilioSid?: string, status: 'sent'|'failed', errorMessage?: string }} params
 */
export async function logSms({ jobId, recipientPhone, recipientType, messageType, messageBody, twilioSid, status, errorMessage }) {
  const db = getDb()
  try {
    // supabase-js does NOT throw on insert errors — it resolves with { error }.
    // Previously this was un-awaited/unchecked, so RLS or auth failures were
    // swallowed silently (handld.sms_log stayed empty in prod even though SMS
    // sent fine). Capture and surface the error so misconfig is visible.
    ...
    if (error) {
      console.error('Failed to log SMS (sms_log insert rejected):', error.message, error)
    }
  } catch (err) {
...
/**
 * Fetch SMS log entries for a specific job.
 * @param {string} jobId
 * @returns {Promise<Array>}
 */
export async function getSmsLogForJob(jobId) {
  const db = getDb()
  const { data, error } = await db
    .from('sms_log')
    .select('*')
